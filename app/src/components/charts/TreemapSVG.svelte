...
{#if showInformation}
  ...
  <Annotation x={currentRegion.x}
    y={currentRegion.y}
    text="Most of the PM<sub>2.5</sub> in <strong>{currentRegion.region}</strong> comes from <strong>{currentRegion.mostPollutingType}</strong> —<strong>{currentRegion.mostPollutingValue.toFixed(2)}</strong> of the <strong>{currentRegion.totalPollutingValue.toFixed(2)}<strong> µg/m<sup>3</sup>"
